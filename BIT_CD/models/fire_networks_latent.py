...
def define_G(args, init_type='normal', init_gain=0.02, gpu_ids=[]):
    if args.net_G == 'base_resnet18':
        ...
        net = BASE_Transformer(input_nc=12, output_nc=2, token_len=4, resnet_stages_num=4,
...
class ResNet(torch.nn.Module):
    def __init__(self, input_nc, output_nc,
                 ...
                 output_sigmoid=False, if_upsample_2x=True, classifier_in_channels=32):
        """
        ...
        self.classifier = TwoLayerConv2d(in_channels=classifier_in_channels, out_channels=output_nc)
...
class BASE_Transformer(ResNet):
    ...
    def __init__(self, input_nc, output_nc, with_pos, resnet_stages_num=5,
                 ...
                 backbone='resnet18',
                 tokenizer_in_channels=32,
                 decoder_softmax=True, with_decoder_pos=None,dim=32,
                 with_decoder=True):
        super(BASE_Transformer, self).__init__(input_nc, output_nc,backbone=backbone,
                                             resnet_stages_num=resnet_stages_num,
                                               if_upsample_2x=if_upsample_2x,classifier_in_channels=dim
                                               )
        self.token_len = token_len
        self.conv_a = nn.Conv2d(tokenizer_in_channels, self.token_len, kernel_size=1,
    ...
    def forward_transformer(self, x1, x2):
        #  forward tokenzier
        if self.tokenizer:
    ...
    # This is the MODIFIED original forward method
    def forward(self, x1, x2):
        # forward backbone resnet
        x1_feat = self.forward_single(x1)
        x2_feat = self.forward_single(x2)

        # forward transformer part
        return self.forward_transformer(x1_feat, x2_feat)
