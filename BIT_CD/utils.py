...
def get_loaders(args):
    ...
    if args.dataset == 'CDDataset':
        DatasetClass = CDDataset
    elif args.dataset == 'FireDataset': # <-- ADD THIS BLOCK
        DatasetClass = FireDataset
    else:
        raise NotImplementedError(
            'Wrong dataset name %s (choose one from [CDDataset, FireDataset])'
            % args.dataset)

    training_set = DatasetClass(root_dir=root_dir, split=split,
                                 img_size=args.img_size,is_train=True,
                                 label_transform=label_transform)
    val_set = DatasetClass(root_dir=root_dir, split=split_val,
