...
from models.modified_autoencoder import ModifiedAutoencoder
...
AE_WEIGHTS_PATH = "/home/zjxeric/projects/def-bereyhia/zjxeric/wildfire-segmentation/model_weights/modified_autoencoder.pth" # IMPORTANT: Use AE weights trained on the fire dataset
SAVE_DIR = "./checkpoints/CD_LatentBIT_Fire_Experiment"
MODEL_SAVE_PATH = os.path.join(SAVE_DIR, "best_latent_bit_fire.pth")
LOG_CSV_PATH = os.path.join(SAVE_DIR, "log_latent_bit_fire.csv")
...
BATCH_SIZE = 8
LEARNING_RATE = 1e-4
IMG_SIZE = 256 # Note: CaBuAr is 512x512, this will be randomly cropped.
IMG_CHANNELS = 12 # 
...
def main():
    ...
    # --- Data Loading ---
    print("Loading CaBuAr (Fire) dataset...")
    
    # Use the path directly
    root_dir = DATASET_ROOT_PATH 
    label_transform = 'norm'

    train_dataset = FireDataset(root_dir=root_dir, split='train', img_size=IMG_SIZE, is_train=True, label_transform=label_transform)
    val_dataset = FireDataset(root_dir=root_dir, split='val', img_size=IMG_SIZE, is_train=False, label_transform=label_transform)
