...
# --- 1. Configuration ---
# ==============================================================================
# --- Paths for the LEVIR-CD experiment ---
DATASET_ROOT_PATH = os.path.expanduser('~/projects/def-bereyhia/zjxeric/data/LEVIR_CD')
SAVE_DIR = "./results/levir_autoencoder"
MODEL_SAVE_PATH = os.path.join(SAVE_DIR, "best_ae_levir.pth")
LOG_CSV_PATH = os.path.join(SAVE_DIR, "ae_levir_log.csv")
os.makedirs(SAVE_DIR, exist_ok=True)

# --- Hyperparameters can remain the same as a starting point ---
NUM_EPOCHS = 100 
BATCH_SIZE = 16
LEARNING_RATE = 1e-4
# ==============================================================================
...
def main():
    ...
    # --- Instantiate the LEVIR_Dataset class ---
    train_dataset = LEVIR_Dataset(root_dir=DATASET_ROOT_PATH, split='train')
    val_dataset = LEVIR_Dataset(root_dir=DATASET_ROOT_PATH, split='val')
    ...
    # The model is instantiated with in_channels=3, which is correct for LEVIR-CD (RGB images)
    autoencoder = ChangeDetectionAE(in_channels=3).to(device)
    optimizer = optim.Adam(autoencoder.parameters(), lr=LEARNING_RATE)
    criterion = nn.MSELoss()
    
    # --- Training and Validation Loop ---
    # The core logic here is identical to the SVCD training script.
    ...
    print("\n--- AE training on LEVIR-CD finished! ---")
